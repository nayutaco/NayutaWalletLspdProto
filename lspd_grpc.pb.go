...
type LightningServiceClient interface {
	...
	// Request time_lock_delta obtained from GetChanInfo() result.
	// (time_lock_delta is the same as cltv_expiry_delta in channel_update message.)
	// If GetChanInfo() result doesn't contain hub's lnrpc.RoutingPolicy, return error.
	//   [GetChanInfo](https://lightning.engineering/api-docs/api/lnd/lightning/get-chan-info)
	GetTimeLockDelta(ctx context.Context, in *GetTimeLockDeltaRequest, opts ...grpc.CallOption) (*GetTimeLockDeltaReply, error)
...
func (c *lightningServiceClient) GetTimeLockDelta(ctx context.Context, in *GetTimeLockDeltaRequest, opts ...grpc.CallOption) (*GetTimeLockDeltaReply, error) {
	out := new(GetTimeLockDeltaReply)
	err := c.cc.Invoke(ctx, "/lspd.LightningService/GetTimeLockDelta", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type LightningServiceServer interface {
	...
	// If GetChanInfo() result doesn't contain hub's lnrpc.RoutingPolicy, return error.
	//   [GetChanInfo](https://lightning.engineering/api-docs/api/lnd/lightning/get-chan-info)
	GetTimeLockDelta(context.Context, *GetTimeLockDeltaRequest) (*GetTimeLockDeltaReply, error)
	/// Register user mail address
...
func (UnimplementedLightningServiceServer) GetTimeLockDelta(context.Context, *GetTimeLockDeltaRequest) (*GetTimeLockDeltaReply, error) {
	return nil, status.Errorf(codes.Unimplemented, "method GetTimeLockDelta not implemented")
}
...
func _LightningService_GetTimeLockDelta_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(GetTimeLockDeltaRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(LightningServiceServer).GetTimeLockDelta(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/lspd.LightningService/GetTimeLockDelta",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(LightningServiceServer).GetTimeLockDelta(ctx, req.(*GetTimeLockDeltaRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
var LightningService_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		{
			MethodName: "GetTimeLockDelta",
			Handler:    _LightningService_GetTimeLockDelta_Handler,
		},
		{
